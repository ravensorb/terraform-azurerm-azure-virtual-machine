...
output "windows_vm_public_ips" {
  ...
  value       = var.public_ip_address_name != null && var.os_flavor == "windows" ? zipmap(azurerm_windows_virtual_machine.win_vm[*].name, azurerm_windows_virtual_machine.win_vm[*].public_ip_address) : null
}
...
output "linux_vm_public_ips" {
  description = "Public IP's map for the all windows Virtual Machines"
  value       = var.public_ip_address_name != null && var.os_flavor == "linux" ? zipmap(azurerm_linux_virtual_machine.linux_vm[*].name, azurerm_linux_virtual_machine.linux_vm[*].public_ip_address) : null
}
